...
const GALLERY = [
  { src: 'https://images.unsplash.com/photo-1541711528-14a5875044d4', alt: 'Sunrise over Cadillac Mountain' },
  { src: 'https://images.unsplash.com/photo-1549887534-1541e9326643', alt: 'Boston Harbor sunset' },
    ...
    <div className='min-h-screen selection:bg-black selection:text-white dark:selection:bg-white dark:selection:text-black'>
      ...
      <header className='sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/70 dark:supports-[backdrop-filter]:bg-black/30 border-b'>
        <div className='container h-16 md:h-20 flex items-center justify-between'>
          ...
          <div className='flex items-center gap-2'>
            ...
            <a href='#contact' className='hidden md:inline-flex'><button className='btn btn-primary gap-2'>{t.book_now} <CalendarCheck2 className='h-4 w-4'/></button></a>
          </div>
        </div>
      </header>
      ...
      <Section id='work' title={t.featured_work} subtitle={t.featured_sub}>
        ...
        <div className='mt-10 text-center'>
          <a href='#contact'><button className='btn btn-outline gap-2'>Request full portfolio <ArrowRight className='h-4 w-4'/></button></a>
        </div>
      </Section>
        ...
        <div className='grid md:grid-cols-3 gap-6'>
          {PACKAGES.map((p)=> (
            <div key={p.name} className={clsx('card p-6 relative', p.featured && 'ring-2 ring-black dark:ring-white')}>
              ...
              <div className='mt-6'>
                <a href='#contact'><button className='btn btn-primary w-full'>Book {p.name}</button></a>
              </div>
            </div>
          ))}
        </div>
      ...
      <Section id='contact' title={t.contact_title} subtitle={t.contact_sub}>
        <div className='grid md:grid-cols-2 gap-8'>
          <div className='card p-6'>
            {/* Simple mailto form fallback (replace later with Formspree endpoint) */}
            <form onSubmit={(e)=>{ e.preventDefault(); const name=e.target.name.value; const email=e.target.email.value; const message=e.target.message.value; window.location.href=`mailto:${CONTACT.email}?subject=Photo Inquiry from ${name}&body=${encodeURIComponent(message + "\n\nReply to: " + email)}` }} className='space-y-4'>
              ...
              <button type='submit' className='btn btn-primary w-full gap-2'>{t.contact_send} <Mail className='h-4 w-4'/></button>
              <p className='text-xs opacity-70'>{t.contact_notice}</p>
            </form>
          </div>
      ...
      <footer className='border-t'>
        <div className='container py-10 text-sm opacity-70 flex flex-col md:flex-row items-center justify-between gap-4'>
          ...
          <div className='flex items-center gap-6'>
            <a href='#work' className='hover:opacity-100 opacity-80'>{t.menu_work}</a>
            <a href='#services' className='hover:opacity-100 opacity-80'>{t.menu_services}</a>
            <a href='#contact' className='hover:opacity-100 opacity-80'>{t.menu_contact}</a>
          </div>
        </div>
      </footer>
    </div>
