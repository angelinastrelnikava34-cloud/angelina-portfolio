...
const CONTENT = {
  ...
  // ── ГАЛЕРЕЯ (Featured Work) ─────────────────────────────────────────────
  // Порядок в массиве = порядок на сайте (сверху слева→направо).
  gallery: [
    // ← Новое фото (оставила ПЕРВЫМ, чтобы попало в верхний ряд)
    {
      src: "/gallery/24-wedding-back-bay-dance-bw.jpeg",
      alt: "Bride and groom dancing in Back Bay, Boston (B&W)",
      focal: "center", // можно '50% 35%' если нужно сдвинуть кадр
    },

    // Твои текущие 12 кадров (оставь, как у тебя назывались файлы).
    // Если имена другие — просто поправь 'src' под свои.
  ...
  // ── ПАКЕТЫ (карточки внизу) ─────────────────────────────────────────────
  // Если нужно — отредактируй цены/описания. Пустые поля можно удалить.
  services: [
    {
  ...
  // Блок «Обо мне» — текст абзацами (каждый элемент массива = отдельный <p>)
  about: {
    paragraphs: [
  ...
  testimonials: [], // при желании заполни позже
};
